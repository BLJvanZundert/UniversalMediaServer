...
public class ImageInfo implements Serializable {
		...
		if (metadata != null) {
			for (Directory directory : metadata.getDirectories()) {
				if (directory instanceof PngDirectory) {
					...
					if (((ExifIFD0Directory) directory).containsTag(ExifIFD0Directory.TAG_BITS_PER_SAMPLE)) {
						bitDepth = ((ExifIFD0Directory) directory).getByteArray(ExifIFD0Directory.TAG_BITS_PER_SAMPLE)[0];
					}
					if (((ExifIFD0Directory) directory).containsTag(ExifIFD0Directory.TAG_SAMPLES_PER_PIXEL)) {
						numComponents = ((ExifIFD0Directory) directory).getInt(ExifIFD0Directory.TAG_SAMPLES_PER_PIXEL);
					}
					if (((ExifIFD0Directory) directory).containsTag(ExifIFD0Directory.TAG_PHOTOMETRIC_INTERPRETATION)) {
						/*
						 * This is "qualified" guessing based on the following assumption:
					     * 0 = WhiteIsZero
					     * 1 = BlackIsZero
					     ...
					     * and is therefore treated as RGB here.
					     */
						switch (((ExifIFD0Directory) directory).getInt(ExifIFD0Directory.TAG_PHOTOMETRIC_INTERPRETATION)) {
							case 2:
							case 3:
							case 6:
								colorSpaceType = ColorSpaceType.TYPE_RGB;
	...
	public ImageInfo copy(Metadata metadata) {
		return new ImageInfo(width, height, format, size, bitDepth, numComponents, colorSpace, colorSpaceType, metadata, imageIOSupport);
	}
