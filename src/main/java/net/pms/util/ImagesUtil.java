...
public class ImagesUtil {
	...
	public static void parseImage(File file, DLNAMediaInfo media) throws IOException {
		...
		try  {
			...
			try {
				...
				if (format == null) {
					throw new UnknownFormatException("Unable to recognize image format for \"" + file.getAbsolutePath() + "\" - parsing failed");
				}
				...
				if (metadata != null) {
					try {
						...
						LOGGER.warn("Unable to parse metadata for \"{}\": {}", file.getAbsolutePath(), e.getMessage());
						LOGGER.trace("", e);
					}
				}
			}
			...
			if (format == null) {
				...
				// Metadata extractor misidentifies some Photoshop created TIFFs for ARW, no reason to log that
				if (format != ImageFormat.ARW || imageInfo.getFormat() != ImageFormat.TIFF) {
					LOGGER.debug(
	 ...
	 * @param outputFormat the {@link ImageFormat} to covert to. If this is
	 ...
	 * @param padToSize Whether padding should be used if source aspect doesn't
	 *                  match target aspect.
	 * @return The scaled, {@code null} if the source is {@code null}.
	 * @throws IOException if the operation fails.
	 */
	...
	public static Image transcodeImage(
		...
		return transcodeImage(inputImage, 0, 0, ScaleType.MAX, outputProfile, updateMetadata, true, dlnaThumbnail, padToSize);
	}
	...
	public static Image transcodeImage(
		...
		return transcodeImage(inputStream, 0, 0, ScaleType.MAX, outputProfile, updateMetadata, true, dlnaThumbnail, padToSize);
	}
	...
	public static Image transcodeImage(
		...
		return transcodeImage(inputByteArray, 0, 0, ScaleType.MAX, outputProfile, updateMetadata, true, dlnaThumbnail, padToSize);
	}
